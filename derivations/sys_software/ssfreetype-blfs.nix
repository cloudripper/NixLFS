{ pkgs, blfsSrcs, blfsHashes, cc2, lib }:
let
  stdenv = pkgs.stdenv;
  ...
  fhsEnv = stdenv.mkDerivation {
    ...
    src = pkgs.fetchurl {
      url = blfsSrcs.freetype;
      sha256 = blfsHashes.freetype;
    };

    # BLFS UEFI/Grub support
    docSrc = builtins.fetchurl {
      url = blfsSrcs.freetype_documentation;
      sha256 = blfsHashes.freetype_documentation;
